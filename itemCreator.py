...
class Button:
    def __init__(self, x, y, width, height, image_path, hover_image_path=None, press_image_path=None, sound_path=None,
                 ...
                 no_normal_name_path=None, no_continue_image_path=None):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
        ...
        self.no_continue_image = self.image
        if no_continue_image_path:
            self.no_continue_image = load_image(no_continue_image_path)
            self.no_continue_image = pygame.transform.scale(self.no_continue_image, (width, height))
    ...
    def drawContinueBtn(self, screen):
        current_image = self.image
        if self.no_continue_image and not consts.continueChecker:
            current_image = self.no_continue_image
        else:
            if self.is_hovered:
                current_image = self.hover_image
            if self.is_pressed:
    ...
    def handle_event(self, event, volS=1, rightCheck=True, continueCheck=True):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and self.is_hovered and not self.is_no_active\
                and (rightCheck or consts.rightNameChecker) and (continueCheck or consts.continueChecker):
