...
class AnimatedTypedText(pygame.sprite.Sprite):
    def __init__(self, sheet, columns, rows, x, y):
        super().__init__(spriteGroups.animatedTypedText)
        self.frames = []
    ...
    def update_game_complete(self, screen, record, stars, repeat_btn, to_lvlmenu_btn, levelTime, levelTimeRect, ln):
        ...
        if self.cur_frame >= numFrames * 10:
            stars.update(record)
            spriteGroups.animatedStars.draw(windows.screen)
...
class AnimatedHero(pygame.sprite.Sprite):
    def __init__(self, sheet, columns, rows, x, y):
        super().__init__(spriteGroups.animatedHero)
        self.frames = []
        self.cut_sheet(sheet, columns, rows)
        self.cur_frame = 0
        self.image = self.frames[self.cur_frame]
...
class AnimatedDots(pygame.sprite.Sprite):
    def __init__(self, sheet, columns, rows, x, y):
        super().__init__(spriteGroups.animatedDots)
        self.frames = []
        self.cut_sheet(sheet, columns, rows)
        self.cur_frame = 0
        self.image = self.frames[self.cur_frame]
